...
// make output dir
fsr.mkdir(path.resolve(__dirname, 'project-dist'), {recursive: true});
...
fsr.mkdir(path.resolve(__dirname, 'project-dist', 'assets'), {recursive: true});

async function copy(src, dest) {
  const files = await fsr.readdir(path.resolve(__dirname, src), {withFileTypes: true});
  files.forEach(async (file) => {
    if (!file.isDirectory()) {
      fsr.copyFile(path.resolve(__dirname, src, file.name), path.resolve(__dirname, dest, file.name));
    } else {
...
copy('assets', 'project-dist/assets');
